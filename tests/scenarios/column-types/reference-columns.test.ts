...
describe('Reference Columns - Real-World Tests', () => {
  ...
  describe('RefList Column - Multiple References', () => {
    ...
    it('should insert project with multiple team members', async () => {
      ...
      // Create project with team - use natural format (plain array)
      const projectIds = await addTestRecords(client, docId, projectsTableId, [
        {
          fields: {
            Name: 'Alpha Project',
            TeamMembers: [alice?.id, bob?.id, carol?.id], // Natural format - plain array
            Budget: 100000
          }
        }
      ])
      ...
      // Verify - Raw client.get() returns Grist's encoded format
      ...
      // Raw Grist API returns encoded format ['L', ...]
      expect(Array.isArray(project?.fields.TeamMembers)).toBe(true)
      expect(project?.fields.TeamMembers).toEqual(['L', alice?.id, bob?.id, carol?.id])
    })
    ...
    it('should handle empty RefList', async () => {
      const projectIds = await addTestRecords(client, docId, projectsTableId, [
        {
          fields: {
            Name: 'Solo Project',
            TeamMembers: [], // Natural format - empty array
            Budget: 50000
          }
        }
      ])
      ...
      const project = projects.records.find((r) => r.id === projectIds[0])
      // Grist returns null for empty RefList, which we decode to null (not empty array)
    ...
    it('should handle RefList with single member', async () => {
      ...
      const projectIds = await addTestRecords(client, docId, projectsTableId, [
        {
          fields: {
            Name: 'Small Project',
            TeamMembers: [alice?.id], // Natural format - plain array with one ID
            Budget: 25000
          }
        }
      ])
      ...
      expect(Array.isArray(project?.fields.TeamMembers)).toBe(true)
      expect(project?.fields.TeamMembers).toEqual(['L', alice?.id])
    })
      ...
      const projectIds = await addTestRecords(client, docId, projectsTableId, [
        {
          fields: {
            ...
            TeamMembers: [alice?.id, bob?.id], // Natural format - plain array
            Budget: 75000
          }
        }
      ])

      // Update to remove Bob and add Carol (using raw API to test direct Grist behavior)
      await client.patch(`/docs/${docId}/tables/${projectsTableId}/records`, {
        records: [
          {
            id: projectIds[0],
            fields: {
              TeamMembers: ['L', alice?.id, carol?.id] // Raw Grist API requires 'L' format
            }
          }
        ]
      })
