...
/**
 * Resource type that can be not found
 */
export type ResourceType = 'document' | 'table' | 'workspace' | 'column' | 'record' | 'organization'

/**
 * Resource not found error with actionable recovery suggestions
 */
export class NotFoundError extends GristError {
  ...
  isRetryable(): boolean {
    return false // Not found won't resolve with retry
  }

  private getSuggestions(): { causes: string[]; nextSteps: string[] } {
