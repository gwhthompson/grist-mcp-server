...
function toColId(raw: string): ColId {
  return raw as ColId;
}
 ...
 */
function approachB_noAssertion(
  tableId: TableId,
  rowIds: (RowId | null)[]
): UserAction {
 ...
 */
function approachA_withAssertion(
  tableId: TableId,
  rowIds: (RowId | null)[]
): UserAction {
...
function test_runtimeBehavior(): void {
  ...
  console.log('Has brand property:', brand in (tableId as any)); // false

  // Equality checks
  console.log('tableId === "MyTable":', tableId === 'MyTable' as any); // true
...
function test_tupleInference(): void {
  ...
  type Inferred1 = typeof tuple1;
  // Inferred: (string | TableId | (RowId | null)[] | {})[]
  ...
  type Inferred2 = typeof tuple2;
  // Inferred: [string, string, (number | null)[], {}]
 ...
 */
function positionSafetyWithAssertions(
  tableId: TableId,
  colId: ColId
): UserAction {
