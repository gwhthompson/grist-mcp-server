 ...
 *
 * ⚠️ IMPORTANT: These helpers are for TESTING GRIST'S RAW OUTPUT ONLY!
 *
 * DO NOT use these to simulate user input in integration tests.
 * Users provide natural formats (plain arrays, ISO strings, etc.).
 * The MCP server handles all encoding/decoding automatically.
 *
 * Use these helpers ONLY for:
 * 1. Verifying what Grist returns (raw API behavior)
 * 2. Unit testing encoding/decoding functions themselves
 * 3. Exploratory tests of Grist's internal API
 *
 * Based on the official GristObjCode specification in docs/reference/grist-types.d.ts
 */
 ...
 */
export function createDict(obj: Record<string, unknown>): CellValue {
 ...
 */
export function createException(errorName: string, ...args: unknown[]): CellValue {
...
export function isReferenceList(
  value: CellValue
): value is [GristObjCode.ReferenceList, string, number[]] {
  return Array.isArray(value) && value[0] === GristObjCode.ReferenceList && value.length === 3
}

export function isDict(value: CellValue): value is [GristObjCode.Dict, Record<string, unknown>] {
...
export function isException(
  value: CellValue
): value is [GristObjCode.Exception, string, ...unknown[]] {
...
export function extractListItems(value: CellValue): unknown[] | null {
...
export function extractReferenceList(
  value: CellValue
): { tableId: string; rowIds: number[] } | null {
  if (!isReferenceList(value)) return null
  return { tableId: value[1], rowIds: value[2] }
}

export function extractDict(value: CellValue): Record<string, unknown> | null {
...
export function validateCellValue(value: unknown): value is CellValue {
  ...
  if (Array.isArray(value)) {
    ...
    // Valid GristObjCode - type assertion safe here after validation
    return Object.values(GristObjCode).includes(value[0] as GristObjCode)
  }
  ...
  if (Array.isArray(value) && value.length > 0) {
    ...
    switch (code) {
      case GristObjCode.List:
        return 'List'
      case GristObjCode.LookUp:
        return 'LookUp'
      case GristObjCode.Dict:
        return 'Dict'
      case GristObjCode.DateTime:
        return 'DateTime'
      case GristObjCode.Date:
        return 'Date'
      case GristObjCode.Skip:
        return 'Skip'
