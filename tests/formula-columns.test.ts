...
import { createReference } from './helpers/cell-values.js';
...
describe('Formula Columns - Real-World Tests', () => {
  ...
  describe('Reference formulas', () => {
    ...
    beforeAll(async () => {
      ...
      customersTableId = await createTestTable(
        client,
        docId,
        'Customers',
        [
      ...
      ordersTableId = await createTestTable(
        client,
        docId,
        'Orders',
        [
    ...
    it('should evaluate reference formulas', async () => {
      ...
      const orders = await client.get<{ records: Array<{ id: number; fields: any }> }>(
    ...
    it('should recalculate when referenced data changes', async () => {
      ...
      // Verify initial calculation
      let orders = await client.get<{ records: Array<{ id: number; fields: any }> }>(
        `/docs/${docId}/tables/${ordersTableId}/records`
      );
      ...
      orders = await client.get<{ records: Array<{ id: number; fields: any }> }>(
        `/docs/${docId}/tables/${ordersTableId}/records`
      );
  ...
  describe('Formula column metadata', () => {
    it('should verify formula column has isFormula flag', async () => {
      ...
      const sumCol = columns.columns.find((c: any) => c.id === 'Sum');
    ...
    it('should verify data columns do not have isFormula flag', async () => {
      ...
      const colA = columns.columns.find((c: any) => c.id === 'A');
  ...
  describe('Formula errors', () => {
    ...
    it('should handle division by zero', async () => {
      ...
      expect(record).toBeDefined();
      expect('DivideByZero' in record!.fields).toBe(true);
    });
  });
});
