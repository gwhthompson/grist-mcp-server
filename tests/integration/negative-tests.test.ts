...
describe('Negative Tests - Validation & Error Detection', () => {
  ...
  describe('B. Choice Constraints', () => {
    ...
    it('B1. should document behavior when value not in choices list', async () => {
      /**
       * TEST: Insert value 'D' when choices are ['A', 'B', 'C']
       * GRIST BEHAVIOR: Document whether Grist accepts, rejects, or coerces
       */

      // Create Choice column with specific choices
      ...
      // DOCUMENT: What happened to 'D'?
      console.log('📝 Grist Choice behavior - value not in list:')
      console.log(`  Test1 (valid 'A'): ${test1?.fields.Status}`)
      console.log(`  Test2 (invalid 'D'): ${test2?.fields.Status}`)

      // Grist typically stores the value even if not in choices
      // It will show as a choice in the UI but not in dropdown
      if (test2?.fields.Status === 'D') {
        console.log('  ✓ Grist ACCEPTS value not in choices list')
      } else if (test2?.fields.Status === '' || test2?.fields.Status === null) {
        console.log('  ✓ Grist REJECTS value not in choices (stores empty)')
      } else {
        console.log(`  ⚠ Grist COERCES value: ${test2?.fields.Status}`)
      }
    })
