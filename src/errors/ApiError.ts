...
export class ApiError extends GristError {
  ...
  getSuggestions(): string[] {
    ...
    switch (this.statusCode) {
      case 400:
        suggestions.push('Check parameter format matches the schema')
        suggestions.push('Call grist_help for parameter documentation')
        if (this.path.includes('/records')) {
          suggestions.push('Verify record data matches column types')
        }
        break

      case 401:
        suggestions.push('Verify GRIST_API_KEY environment variable is set')
        suggestions.push('Check if API key has expired')
        suggestions.push('Generate a new API key from your Grist profile settings')
        break

      case 403:
        suggestions.push('Use grist_get_documents to verify access to the document')
        suggestions.push('Check that API key has write permissions for this operation')
        if (this.method !== 'GET') {
          suggestions.push('Request editor or owner access from the document owner')
        }
        break

      case 404:
        if (this.path.includes('/docs/')) {
          suggestions.push('Use grist_get_documents to find valid document IDs')
        }
        if (this.path.includes('/tables/')) {
          suggestions.push('Use grist_get_tables to list available tables')
        }
        if (this.path.includes('/records')) {
          suggestions.push('Use grist_get_records to verify row IDs exist')
        }
        suggestions.push('Resource may have been deleted - check with list operations')
        break

      case 429:
        suggestions.push('Wait 60 seconds before retrying')
        suggestions.push('Reduce request frequency')
        suggestions.push('Use grist_upsert_records to batch multiple operations')
        break
      ...
      case 503:
      case 504:
        suggestions.push('Retry the request in a few seconds')
        suggestions.push('If persistent, check Grist server status')
        break

      default:
        suggestions.push('Review error message for details')
        suggestions.push('Call grist_help for operation documentation')
    }
